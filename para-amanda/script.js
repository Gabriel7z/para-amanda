(function () {
  ...
  function montarFotos() {
    ...
    CONFIG.fotos.forEach((foto, i) => {
      album.push({ src: foto.src, legenda: foto.legenda });
  ...
  function preencherPolaroid(fig, arquivo) {
    ...
    const indice = $$("#polaroides .polaroid").indexOf(fig);
    if (indice >= 0) {
      album[indice] = album[indice] || { legenda: img.alt };
      album[indice].src = img.src;
      album[indice].legenda = img.alt;
    }
  }
